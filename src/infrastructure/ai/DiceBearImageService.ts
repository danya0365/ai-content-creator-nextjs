...
export class DiceBearImageService implements IImageService {
  ...
  private async executeGeneration(
    ...
    try {
      ...
      const seed = encodeURIComponent(prompt.replace(/[^a-zA-Z0-9ก-ฮ]/g, ""));
      ...
      if (!response.ok) {
        console.error(
          "[DiceBearImageService] Failed to fetch image:",
          response.status,
        );
        return {
          success: false,
      ...
      console.log(
        "[DiceBearImageService] Image downloaded, base64 length:",
        base64Data.length,
      );
